...
import { getTelegramSupabase } from "./supabase";

function supabase() {
  return getTelegramSupabase() as any;
}
...
/** URLs for Telegram (image_urls[] with legacy image_url fallback). */
export function productImageUrls(row: { image_url: string | null; image_urls: string[] | null }): string[] {
  const urls = (row.image_urls ?? []).filter((u) => typeof u === "string" && u.trim());
  if (urls.length) return urls.map((u) => u.trim());
...
export async function upsertUserFromTelegram(tg: {
  ...
  // Link web checkout → bot account.
  // If a guest order created a user row with the same telegram_username but a placeholder telegram_user_id,
  // update it to the real Telegram ID before we upsert on telegram_user_id.
  const handle = tg.username?.trim() || null;
  if (handle) {
    try {
      const { data: existingRaw } = await supabase()
        ...
        .maybeSingle();
      const existing = existingRaw as unknown as { id: string; telegram_user_id: number } | null;

      if (existing?.id && existing.telegram_user_id !== tg.id) {
        await supabase()
          .from("users")
          .update({
...
export async function listActiveProducts(
  limit = 12,
  offset = 0
): Promise<Array<{ id: string; title: string; description: string | null; image_url: string | null; image_urls: string[] | null }>> {
  const { data, error } = await supabase()
    .from("products")
    .select("id, title, description, image_url, image_urls")
    .eq("active", true)
...
export async function listActiveProductsByCategory(
  ...
  limit = 12,
  offset = 0
): Promise<Array<{ id: string; title: string; description: string | null; image_url: string | null; image_urls: string[] | null; active: boolean }>> {
  const { data, error } = await supabase()
    .from("product_categories")
    .select("products!inner(id, title, description, image_url, image_urls, active)")
  ...
  return ((data ?? []).flatMap((row: { products: ProductRow[] }) => row.products ?? []) ?? []) as any;
}
...
export async function listActiveProductsByMainCategory(mainCategoryId: string, limit = 12, offset = 0) {
  const { data: catRows, error: e1 } = await supabase()
    ...
    .eq("active", true);
  if (e1) throw e1;
  const categoryIds = ((catRows ?? []) as Array<{ id: string }>).map((c) => c.id);
  ...
  const flat: ProductRow[] = ((data ?? []) as any).flatMap((row: { products: ProductRow[] }) => row.products ?? []);
  const seen = new Set<string>();
  const unique = flat.filter((p: ProductRow) => {
    if (seen.has(p.id)) return false;
    seen.add(p.id);
    return true;
  });
  unique.sort((a: ProductRow, b: ProductRow) => a.title.localeCompare(b.title));
  return unique.slice(offset, offset + limit) as any;
}

export async function listActiveMainCategories(): Promise<Array<{ id: string; name: string; slug: string }>> {
  const { data, error } = await supabase()
...
export async function listActiveCategoriesByMain(
  mainCategoryId: string
): Promise<Array<{ id: string; name: string; slug: string; main_category_id: string }>> {
  const { data, error } = await supabase()
    .from("categories")
...
export async function getProduct(
  productId: string
): Promise<{ id: string; title: string; description: string | null; image_url: string | null; image_urls: string[] | null; active: boolean }> {
  const { data, error } = await supabase()
    .from("products")
    .select("id, title, description, image_url, image_urls, active")
    .eq("id", productId)
    .single();
  if (error) throw error;
  return data as any;
...
export async function getMainCategoryName(mainCategoryId: string): Promise<string> {
  const { data, error } = await supabase().from("main_categories").select("name").eq("id", mainCategoryId).single();
  if (error) throw error;
...
export async function listActiveVariants(
  productId: string
): Promise<Array<{ id: string; size: string; price_cents: number; currency: string; stock: number }>> {
  const { data, error } = await supabase()
    .from("product_variants")
...
export async function createOrderItems(orderId: string, items: Array<{ variantId: string; qty: number }>) {
  ...
  const { data: variants, error: e1 } = await supabase().from("product_variants").select("id, price_cents").in("id", variantIds);
  if (e1) throw e1;
  const typedVariants = (variants ?? []) as Array<{ id: string; price_cents: number }>;

  const priceById = new Map<string, number>(typedVariants.map((v: { id: string; price_cents: number }) => [v.id, v.price_cents]));
...
export async function listOrdersForUser(
  userId: string,
  limit = 10
): Promise<Array<{ id: string; status: string; payment_method: string; total_cents: number; currency: string; created_at: string }>> {
  const { data, error } = await supabase()
    .from("orders")
...
export async function setOrderStripeSession(orderId: string, sessionId: string) {
  const { error } = await supabase().from("orders").update({ stripe_checkout_session_id: sessionId }).eq("id", orderId);
  if (error) throw error;
}
