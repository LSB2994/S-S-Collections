...
export function createBot() {
  ...
  bot.start(async (ctx) => {
    await ctx.reply(
      "Welcome! Use **Shop** and **Cart** at the bottom, or the buttons below.\n\nTip: open Shop → section → category → pick a product → choose a size.",
      { parse_mode: "Markdown", ...mainNavKeyboard }
    );
    await ctx.reply(
      "Quick links:",
...
async function sendCatalogByMain(ctx: BotContext, mainCategoryId: string, offset: number) {
  ...
  for (const p of products) {
    ...
    if (p.image_url) await ctx.replyWithPhoto({ url: p.image_url }, { caption: desc, ...kb });
...
async function sendProduct(ctx: BotContext, productId: string) {
  ...
  if (p.image_url) await ctx.replyWithPhoto({ url: p.image_url }, { caption: text, ...kb });
...
async function createOrderFromCart(ctx: BotContext, method: "cod" | "stripe") {
  ...
  await ctx.reply(
    `Order created: #${orderId}\nStatus: awaiting_payment\n\nOpen the payment link below:`,
    Markup.inlineKeyboard([[Markup.button.url("Pay with Stripe", `${env.PUBLIC_BASE_URL}/pay/${orderId}`)]])
  );
}
